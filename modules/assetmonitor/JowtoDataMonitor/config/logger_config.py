...
"""
    auth: sunhaobo
    version: v1.0
    function: 统一的日志输出配置文件 适用模块 JowtoDataMonitor
    date: 2025.11.24
    note:
        1、引入
...
def setup_logger():
    ...
    log_dir_path = os.path.abspath(current_abs_path_dir) + '/../../../../log/'   # 20251124 目录归档，又添加了一级目录

    # 确保日志目录存在
    if not os.path.exists(log_dir_path):
        os.makedirs(log_dir_path)

    logger = logging.getLogger('jowto_monitor')
    ...
    if not logger.hasHandlers():
        ...
        file_handler = RotatingFileHandler(
            log_dir_path + "/jowto_monitor.log",
